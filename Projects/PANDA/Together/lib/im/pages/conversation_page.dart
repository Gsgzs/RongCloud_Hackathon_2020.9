...
class _ConversationPageState extends State<ConversationPage>
    ...
    try {
      ...
      if (response.data["code"] == 200) {
        ...
        if (response.data["data"]["name"] != targetName ||
          ...
          user.id = arguments["targetId"];
          user.name = response.data["data"]["name"];
          user.portraitUrl = response.data["data"]["url"];
          example.UserInfoDataSource.setUserInfo(user);
          print('---已更新---');
        } else {
          print('---无更新---');
        }
  ...
  void initState() {
    ...
    // titleContent = '与 $targetName 的会话';
    titleContent = '$targetName';
  ...
  Widget _getExtentionWidget() {
    if (currentInputStatus == InputBarStatus.Extention) {
      ...
      return Container(
          height: RCLayout.ExtentionLayoutPhraseWidth,
          child: _buildPhrasesList());
    } else if (currentInputStatus == InputBarStatus.Emoji) {
      return Container(
          height: RCLayout.ExtentionLayoutEmojiWidth, child: _buildEmojiList());
    } else {
  ...
  void _initExtentionWidgets() {
    ...
    if (conversationType == RCConversationType.Private) {
      Widget secretChatWidget = WidgetUtil.buildExtentionWidget(
        ...
        isSecretChat = !isSecretChat;
        String contentStr = isSecretChat ? "隐私回复已开启" : "隐私回复已关闭";
        print(contentStr);
  ...
  void didTapForward() async {
    ...
    for (int msgId in selectedMessageIds) {
      ...
      if (forwardMsg.content == null ||
        ...
        DialogUtil.showAlertDiaLog(context, "无法识别的消息、隐私回复消息以及未发送成功的消息不支持转发");
        return;
      }
